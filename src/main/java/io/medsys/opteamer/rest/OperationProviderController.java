...
import io.medsys.opteamer.service.OperationProviderService;
...
public class OperationProviderController {
    ...
    public ResponseEntity<OperationProviderDTO> getOperationProviderById(@PathVariable(name = "type") String type) {
        Optional<OperationProviderDTO> operationProviderDTOOptional = operationProviderService.getOperationProviderById(type);
    ...
    @PutMapping("/{type}")
    public ResponseEntity<OperationProviderDTO> updateOperationProvider(@PathVariable(name = "type") String type, @RequestBody OperationProviderDTO operationProviderDTO) {
        Optional<OperationProviderDTO> operationProviderDTOOptional = operationProviderService.updateOperationProvider(type, operationProviderDTO);
        return operationProviderDTOOptional.map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND).build());
    }
